...
public class EmployerManager implements EmployerService {
	...
	@Autowired
	public EmployerManager(EmployerDao employerDao, UserDetailsDao userDetailsDao, RoleService roleService, LoginAndRoleService loginAndRoleService) {
	...
	public Result add(EmployerRegistryDTO registryDTO) {
		if(registryDTO.getPassword().length() >= 8 && registryDTO.getEmployer().getEmail().length() >= 8) {
			...
			userDetails.setPassword(registryDTO.getPassword());
